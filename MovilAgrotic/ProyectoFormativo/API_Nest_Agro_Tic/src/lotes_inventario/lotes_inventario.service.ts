...
export class LotesInventarioService {
  ...
  async search(query: string, page: number = 1, limit: number = 10) {
    ...
    try {
      ...
      for (const lote of lotes) {
        ...
        const availableInLote = Math.round((cantidadDisponible + cantidadParcial - cantidadReservadaActiva) * 100) / 100;
  ...
  async findAllPaginated(page: number = 1, limit: number = 10) {
    ...
    const itemsWithCalculatedQuantities = items.map((item) => {
      ...
      const cantidadDisponibleParaReservar = Math.round((cantidadDisponible + cantidadParcial - cantidadReservadaActiva) * 100) / 100;
      const stockTotal = Math.round((cantidadDisponible + cantidadParcial) * 100) / 100;
