...
export class ReservasXActividadService {
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      // First, delete all inventory movements associated with this reservation
      console.log(`Deleting inventory movements for reservation ${id}`);
      await this.movimientosInventarioRepo.delete({ fkReservaId: id });
      
      // Now remove the reservation (should work since all related records are deleted)
      await this.reservasXActividadRepo.remove(entity);
      console.log(`Reservation ${id} successfully removed`);
    } catch (error) {
