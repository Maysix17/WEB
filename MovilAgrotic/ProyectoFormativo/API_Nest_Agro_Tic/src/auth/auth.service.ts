...
import { CentralizedConfigService } from '../common/config/centralized-config.service';
...
*/
  ...
  async forgotPassword(
    ...
    if (platform === 'mobile') {
      // Usar enlace web que redirige al deep link para móvil
      const apiUrl = this.centralizedConfigService.getApiUrl();
      resetLink = `${apiUrl}/auth/reset-password-mobile?token=${token}`;
    } else {
      // Enlace web para otros casos
      const frontendUrl = this.centralizedConfigService.getFrontendUrl();
    ...
    try {
      ...
      this.logger.log(`Email de reseteo enviado a: ${usuario.correo} (platform: ${platform || 'web'})`);
    } catch (error) {
      this.logger.error(
